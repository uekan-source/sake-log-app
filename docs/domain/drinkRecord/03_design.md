...
- **照会が失敗した写真からの登録は、仮の名前（例：「銘柄不明 8/21」）・`kind = IS_OTHER` の未確定 Brand で受ける**──圏外や読み取り不能でも写真と記録を残す。後で再照会または手直しする。代償として **IS_OTHER は「その他の酒」と「不明の仮置き」の二義を背負う**が、仮置きの行は未確定の印が付いているので見分けられる（Q23）
...
| 4 | 1 | IS_OTHER | 銘柄不明 8/21 | | | | | IS_UNCONFIRMED |

（1 行目＝確認済みのワイン。2 行目＝AI 推定のままの日本酒。3 行目＝手入力の「その他」──作成時から確定。4 行目＝照会失敗の仮置き──IS_OTHER＋未確定で受け、後で直す）
...
- **写真の 1:1 所有はスキーマでも守る**：写真 URL の列に一意制約を張り、同じファイルを 2 つの記録が指せないようにする（共有を許すと、片方の記録の削除でもう片方の写真ファイルが道連れに消える）。「URL と出所は両方あるか・両方ないか」も、型（`Photo`）では守れているが DB 上は nullable 2 列になるため、CHECK 制約で運用経路（手書き SQL 等）からも守る
...
- **削除は連鎖しない**：記録を消しても銘柄は消えない。記録 0 件になった銘柄のみ、本人の操作（銘柄の一覧から。Q22）で削除できる
- 写真だけを外すこともできる（`photo` を None にし、ファイルも消す）。記録本体は残る
...
- 照会は結果を**その場で返す**。店で飲んでいる間（数分以内）に見られることを前提とする（Q4 の利用場面：注文 → 撮る → 飲みながらチェック）
- **照会には 1 本の酒について複数枚の写真を使える**（表＋裏ラベルなど。品種・産地は裏に書いてあることが多い。Q24）。照会は保存しないので複数枚を受けてもモデルは変わらない。**保存する写真は 1 枚**で、記録に付ける 1 枚を選ぶ（既定は 1 枚目）
